...
import { getUserRaffleHistory, getUserProfileStats } from '@/lib/prismaFunctions';
...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-12">
      ...
      {pageState === 'success' && stats && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-12">
          ...
          <div className="border-2 border-amber-400 border-opacity-30 rounded p-6 bg-black bg-opacity-50">
            ...
            <p className="text-3xl font-bold text-amber-400">{stats.totalWagered}</p>
          ...
          <div className="border-2 border-green-500 border-opacity-30 rounded p-6 bg-black bg-opacity-50">
            ...
            <p className="text-3xl font-bold text-green-400">{stats.totalWinnings}</p>
            <p className="text-xs text-gray-500 mt-1">cUSD</p>
          </div>
        </div>
      )}
      ...
      {/* History Section */}
      <div>
        <h2 className="text-3xl font-bold text-amber-400 mb-6">📊 Ticket History</h2>
        ...
        <div className="flex gap-2 mb-8 border-b border-amber-400 border-opacity-20 overflow-x-auto pb-4">
          {(['all', 'active', 'won', 'lost'] as TicketFilter[]).map((tab) => (
            ...
            >
              ...
              {tab === 'active' && `🎰 Active (${tickets.filter((t) => t.status === 'active').length})`}
              {tab === 'won' && `🎊 Won (${tickets.filter((t) => t.status === 'won').length})`}
              {tab === 'lost' && `❌ Lost (${tickets.filter((t) => t.status === 'lost').length})`}
            </button>
          ))}
        </div>
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-center">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-white">{ticket.raffle.title}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="font-bold text-amber-400">{ticket.amount} cUSD</p>
                  </div>
